...
namespace STK.Application.Handlers
{
    ...
    public class GetUserCreatedOrganizationsHandler : IRequestHandler<GetUserCreatedOrganizationsQuery, UserCreatedOrganizationsResultDto>
    {
        ...
        public async Task<UserCreatedOrganizationsResultDto> Handle(GetUserCreatedOrganizationsQuery query, CancellationToken cancellationToken)
        {
            try
            {
                // 1) Все организации, добавленные текущим пользователем
                var orgIds = await _dataContext.UserCreatedOrganizations
                    .AsNoTracking()
                    .Where(uc => uc.UserId == query.UserId)
                    .Select(uc => uc.OrganizationId)
                ...
                // 3) ИНН незагруженных организаций (IsLoad == false),
                //    которые были ДОБАВЛЕНЫ текущим пользователем (есть связь в UserCreatedOrganizations)
                var notLoadedInns = await _dataContext.UserCreatedOrganizations
                    .AsNoTracking()
                    .Where(uc => uc.UserId == query.UserId)
                    .Join(
                        _dataContext.OrganizationDownload
                            .AsNoTracking()
                            .Where(od => !od.IsLoad && !string.IsNullOrWhiteSpace(od.Inn)),
                        uc => uc.OrganizationId,
                        od => od.Id,
                        (uc, od) => od.Inn!
                    )
                    .Distinct()
